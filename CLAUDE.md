...
**Verifying a JSON-serialising pipeline refactor is behaviour-preserving: compare parsed objects, not raw text.** When refactoring anything that feeds `_js_data()` -> `const NS = [...]` (e.g. the 2026-09-25 `load_namespace()` split, or the camelCase-rename-table extraction), diff the *old vs. new generated HTML as text* only tells you whether anything changed — it can't tell you whether a change is safe. A data-driven rewrite (a dict built from a table instead of ~70 hand-written lines, say) legitimately reorders JSON object keys, which a raw-text diff reports as a difference even though key order is semantically irrelevant to any JS consumer. Instead, extract and `json.loads()` both outputs' `const NS = [...]` block and compare the resulting Python objects for equality — that's order-independent and actually proves the refactor preserved behaviour.
